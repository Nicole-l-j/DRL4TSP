...
warnings.filterwarnings("ignore",
                        message="indexing with dtype torch.unit8 is now deprecated, please use a dtype torch.bool instead")
...
def eval_dataset(dataset_path, decode_strategy, width, softmax_temp, opts):
    ...
    print('Total durations: {}s'.format(np.sum(durations)/opts.batch_size))
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar, ascii=True):
        ...
        with torch.no_grad():
            if decode_strategy in ('sample', 'greedy'):
                ...
                sequences, costs = model.sample_many(nodes, graph, batch_rep=batch_rep,
                                                     iter_rep=iter_rep, n_paths=n_paths)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', type=int, default=128,
                        help="Batch_size to use during (baseline) evaluation")
    ...
    assert opts.o is None or (len(opts.datasets) == 1 and len(opts.widths) <= 1), \
    ...
    for decode_strategy, width in zip(opts.decode_strategies, opts.widths):
        for path_index, dataset_path in enumerate(opts.datasets):
            results = eval_dataset(dataset_path, decode_strategy, width, opts.softmax_temperature, opts)
