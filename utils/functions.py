...
def sample_many(inner_func, input, batch_rep=1, iter_rep=1, n_paths=None):
    """
    ...
    for i in range(iter_rep):
        _, pi, cost = inner_func(input, n_paths)
        # pi.view(-1, batch_rep, pi.size(-1))
        # cost, mask = get_cost_func(input, pi)
        pi = torch.stack(pi, 1)
        cost = torch.stack(cost, 1)
        cost, indices = torch.sort(cost, dim=1)
        for j in range(0, len(pi)):
            pis_inner.append(torch.index_select(pi[j], 0, indices[j, :]))
        pi = torch.stack(pis_inner, 0)[:, 0]
        cost = cost[:, 0]
...
def get_best(sequences, cost, ids=None, batch_size=None):
    """
    Ids contains [0, 0, 0, 1, 1, 2, ..., n, n, n] if 3 solutions found for 0th instance, 2 for 1 st, etc
