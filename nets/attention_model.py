...
class AttentionModel(nn.Module):
    ...
    def forward(self, nodes, graph, opts=None, baseline=None, bl_val=None, return_pi=False):
        """
        ...
        costs, lls, losses, pi, outputs = self._inner(nodes, graph, embeddings, baseline, bl_val, self.n_paths)

        costs = torch.stack(costs, 1)
        lls = torch.stack(lls, 1)

        if baseline is not None:
            losses = torch.stack(losses, 0)
            # return costs, ll, losses

        if return_pi:
            return costs, lls, pi, losses

        return costs, lls

    def _inner(self, nodes, graph, embeddings, baseline, bl_val, n_paths):
        costs, lls = [], []
        outputs = []
        losses = []

        # Create problem state for masking (tracks which nodes have been visited)
        states = [self.problem.make_state(nodes, graph) for i in range(n_paths)]
        ...
        for i in range(n_paths):
            ...
            j = 0  # The index j represent decode step
            while not (self.shrink_size is None and states[i].all_finished()):

                if self.shrink_size is not None:
                    unfinished = torch.nonzero(states[i].get_finished() == 0)
                    if len(unfinished) == 0:
                        break
                    unfinished = unfinished[:, 0]
                    # Check if we can shrink by at least shrink_size and if this leaves at least 16
                    # (otherwise batch norm wil not work well and it is inefficient anyway)
                    if 16 <= len(unfinished) <= states[i].ids.size(0) - self.shrink_size:
                        # Filter states
                        states[i] = states[i][unfinished]
                ...
                if self.shrink_size is not None and states[i].ids.size(0) < batch_size:
                    log_p_, selected_ = log_p, selected
                    log_p = log_p_.new_zeros(batch_size, *log_p_.size()[1:])
                    selected = selected_.new_zeros(batch_size)

                    log_p[states[i].ids[:, 0]] = log_p_
                    selected[states[i].ids[:, 0]] = selected_
            ...
            # Get all the steps' results of one decoder
            _log_p = torch.stack(output, 1)
            pi = torch.stack(sequence, 1)

            outputs.append(_log_p)

            # Get predicted costs of one decoder
            ...
            if baseline is not None:
                if i == 0:
                    bl_val, bl_loss = baseline.eval(nodes, graph, costs[0]) if bl_val is None else (bl_val, 0)
                reinforce_loss = ((cost - bl_val) * ll).mean()
                loss = reinforce_loss + bl_loss
                losses.append(loss)

        # return results of n_paths decoders
        return costs, lls, losses, pi, outputs

    def beam_search(self, *args, **kwargs):
        return self.problem.beam_search(*args, **kwargs, model=self)

    def precompute_fixed(self, nodes, graph, path_index=None):
        embeddings = self.embedder(self._init_embed(nodes), graph)

        # Use a CachedLookup such that if we repeatedly index this object with the same index we only need to do
        # the lookup once... this is the case if all elements in the batch have maximum batch size
        return CachedLookup(self._precompute(embeddings, num_steps=1, path_index=path_index))
    ...
    def sample_many(self, input, graph, batch_rep=1, iter_rep=1, n_paths=None):
        """
        ...
        return sample_many(
            ...
            lambda input, n_paths: self._inner(*input, n_paths),
    ...
    def _one_to_many_logits(self, query, glimpse_K, glimpse_V, logit_K, mask, path_index):
        ...
        if self.mask_inner:
            ...
            # better not set the -inf, otherwise, we can not get the kl_divergernce loss later
            compatibility[mask[None, :, :, None, :].expand_as(compatibility)] = -math.inf
            # compatibility[mask[None, :, :, None, :].expand_as(compatibility)] = -1e-10
